...
class _AnalyticsPageState extends ConsumerState<AnalyticsPage> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    final dataset = _getDataset();
    final realRole = ref.watch(appStateProvider).role;
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final cards = switch (role) {
          ...
          UserRole.buyer => ref.watch(appStateProvider).buyerSubRole == BuyerSubRole.retailerWholesaler
              ? (isDemo
                  ? [
                      _kpiCard('Contracted Volume', '1,480 MT', '+240 MT', Icons.inventory_2_outlined, AnalyticsPage.green, width),
                      _kpiCard('Escrow Locked', '\$412,500 USD', '3-Tranche Vault', Icons.shield_outlined, AnalyticsPage.blue, width),
                      _kpiCard('Landed Savings', '14.2% vs Spot', 'Direct Farmgate', Icons.trending_down, AnalyticsPage.orange, width),
                      _kpiCard('Supplier SLA', '98.6% Pass', 'Moisture & Brix', Icons.verified_outlined, AnalyticsPage.purple, width),
                    ]
                  : [
                      _kpiCard('Contracted Volume', '${dataset.orders} MT', '+0.0%', Icons.inventory_2_outlined, AnalyticsPage.green, width),
                      _kpiCard('Escrow Locked', dataset.revenue, '+0.0%', Icons.shield_outlined, AnalyticsPage.blue, width),
                      _kpiCard('Landed Savings', '12.0%', '+0.0%', Icons.trending_down, AnalyticsPage.orange, width),
                      _kpiCard('Supplier SLA', dataset.fulfillment, '+0.0%', Icons.verified_outlined, AnalyticsPage.purple, width),
                    ])
              : (isDemo
          ...
          UserRole.consumer => isDemo
              ? [_kpiCard('Grocery Basket', '\$185 USD', '-14.0% Farmgate', Icons.shopping_bag_outlined, AnalyticsPage.green, width), _kpiCard('Delivery On-Time', '99.1%', '+2.0%', Icons.electric_moped_outlined, AnalyticsPage.blue, width), _kpiCard('Freshness Index', '96.4%', 'Harvested <24h', Icons.eco_outlined, AnalyticsPage.orange, width), _kpiCard('Carbon Offset', '18.4 kg', 'Direct Farm', Icons.nature_outlined, AnalyticsPage.purple, width)]
              : [_kpiCard('Grocery Basket', '\$0 USD', '+0.0%', Icons.shopping_bag_outlined, AnalyticsPage.green, width), _kpiCard('Delivery On-Time', '100%', '+0.0%', Icons.electric_moped_outlined, AnalyticsPage.blue, width), _kpiCard('Freshness Index', '100%', 'Verified', Icons.eco_outlined, AnalyticsPage.orange, width), _kpiCard('Carbon Offset', '0 kg', 'Direct', Icons.nature_outlined, AnalyticsPage.purple, width)],
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final advisory = isDemo
        ? switch (role) {
            ...
            UserRole.buyer => (title: 'Strategic Bulk Sourcing & Outgrower Forward Price Advisory', text: 'Masvingo and Mazowe outgrowers reporting 3,200 MT fresh White Maize and Soybeans arrivals. Spot prices are \$275/MT vs \$310/MT forward forecast. Recommended action: Lock 3-tranche forward escrow contracts now before regional mill demand spikes.'),
            UserRole.consumer => (title: 'Farmgate Fresh Produce Price Drop Alert', text: 'Tomato spot rates dropped -18% across Harare direct hubs due to peak harvesting in Mashonaland West. Add to cart today for same-day InDrive doorstep delivery.'),
        ...
        : switch (role) {
            ...
            UserRole.buyer => (title: 'Real-Time Commercial B2B Sourcing Engine Active', text: 'Multi-ton outgrower forward contracts connected. Machine-learning algorithms are monitoring SADC price arbitrage and weighbridge quality grading.'),
            UserRole.consumer => (title: 'Real-Time Farmgate Fresh Store Active', text: 'Direct farm-to-doorstep produce fresh stream connected. Real-time InDrive route tracking and instant grocery pricing active.'),
